...
namespace MyStudyHelper.Classes.Backend
{
    public class AccountBackend
    {
        ...
        public string CheckInfo(string uname, string email, string pword1, string pword2)
        {
            var MinLength = 12;
            ...
            if (String.IsNullOrWhiteSpace(uname) || String.IsNullOrWhiteSpace(email)
                ...
                return "Please fill in all required field(s) with valid updated information, cannot be left empty";
        ...
        public async Task<User> Update(string uname, string email, string name, string pword)
        {
